...
Docker 컨테이너는 기본적으로 독립된 network namespace 안에서 실행된다. 컨테이너 내부에서 보이는 `eth0`는 물리 NIC가 아니라 veth pair의 컨테이너 쪽 endpoint다. 이 `eth0`와 연결된 host 쪽 veth peer가 `docker0` bridge에 붙는다.
...
```text
container process
  -> container eth0, veth pair의 한쪽 끝
  -> host-side veth peer
  -> docker0 bridge
  -> host routing
  -> source NAT
...
실습 문서: [09-mini-docker.md](./practices/09-mini-docker.md)
...
### Docker와 미니 Docker의 차이

학습용 미니 Docker는 컨테이너의 핵심 원리를 이해하기 위한 실험이다. 실제 Docker처럼 image pull, registry, layer cache, volume driver, network driver, restart policy, logging driver, seccomp/AppArmor/SELinux profile, rootless mode, API server를 모두 구현하는 것은 아니다.

| 범위 | 미니 Docker 실험 | 실제 Docker |
| --- | --- | --- |
| root filesystem | 직접 준비한 디렉터리 | image layer와 snapshotter/storage driver |
| process isolation | `unshare`, namespace | OCI runtime, containerd, runc |
| root 전환 | `chroot` 또는 `pivot_root` | mount namespace와 rootfs setup 자동화 |
| `/proc` | 직접 mount | runtime이 namespace에 맞춰 mount |
| network | veth, bridge, route 직접 구성 | Docker network driver, IPAM, DNS, iptables/nftables |
| resource limit | cgroup 파일 직접 조작 | cgroup driver와 runtime spec |
| 보안 profile | 대부분 직접 처리하지 않음 | capability, seccomp, LSM, user namespace |

### Namespace 조합

Linux namespace는 전역 리소스를 격리된 것처럼 보이게 하는 커널 기능이다. `unshare`는 현재 프로세스를 새 namespace로 분리한 뒤 command를 실행할 수 있게 해준다.

| namespace | 격리 대상 | 실험 포인트 |
| --- | --- | --- |
| UTS | hostname, domain name | 컨테이너별 hostname 확인 |
| PID | process ID 공간 | 내부 PID 1과 `/proc` mount 확인 |
| mount | mount table | host mount table을 오염시키지 않도록 분리 |
| network | interface, route, port, firewall table | veth/bridge를 직접 연결 |
| IPC | System V IPC, POSIX message queue | process 간 IPC 격리 확인 |
| user | UID/GID mapping | rootless 실험과 capability 획득 |
| cgroup | cgroup root view | `/proc/<pid>/cgroup` 관찰 범위 |

PID namespace는 `--fork`와 함께 쓰는 것이 중요하다. 새 PID namespace를 만든 프로세스의 child가 그 namespace의 PID 1이 되기 때문이다. PID namespace에서 정확한 process 목록을 보려면 `/proc`도 새 namespace 기준으로 mount해야 한다.

mount namespace를 만들 때는 mount propagation도 고려해야 한다. host mount가 공유 propagation 상태이면 실험 중 mount/unmount가 예상 밖으로 전파될 수 있다. 학습용 실험에서는 새 mount namespace 안에서 `mount --make-rprivate /`를 먼저 적용하는 패턴이 안전하다.

### root filesystem 전환

`chroot`는 프로세스가 보는 root directory를 바꾸지만, 그 자체만으로 완전한 보안 격리는 아니다. 이미 열린 file descriptor, mount namespace, capability, device 접근, `/proc` 노출 상태에 따라 빠져나갈 여지가 있다.

`pivot_root`는 새 root filesystem으로 root를 전환하고 old root를 별도 위치로 밀어낸다. 실제 컨테이너 runtime은 mount namespace 안에서 rootfs를 구성하고 `pivot_root` 또는 유사한 방식으로 process의 root를 전환한다.

학습 순서는 단순한 `chroot`부터 시작하고, mount namespace와 `/proc` mount의 의미를 이해한 뒤 `pivot_root`를 별도 단계로 보는 편이 좋다.

### cgroup 제한

cgroup은 namespace와 역할이 다르다. namespace는 “무엇이 보이는가”를 바꾸고, cgroup은 “얼마나 쓸 수 있는가”를 제한하고 계측한다. cgroup v2에서는 단일 unified hierarchy 아래에서 controller를 활성화하고, process를 cgroup에 넣은 뒤 `memory.max`, `cpu.max`, `pids.max` 같은 파일로 제한을 건다.

| cgroup v2 파일 | 의미 |
| --- | --- |
| `cgroup.controllers` | 사용 가능한 controller 목록 |
| `cgroup.subtree_control` | child cgroup에 활성화할 controller |
| `cgroup.procs` | 이 cgroup에 속한 process |
| `memory.max` | memory 상한 |
| `cpu.max` | CPU quota/period |
| `pids.max` | 생성 가능한 process 수 제한 |

수동 cgroup 실험은 host 전체 cgroup tree에 영향을 줄 수 있다. systemd가 관리하는 환경에서는 임의로 root cgroup을 수정하기보다 실험용 subtree를 만들고 정리 명령을 명확히 둔다.

### 네트워크 직접 구성

Docker 기본 bridge network는 network namespace, veth pair, Linux bridge, IPAM, route, NAT를 자동으로 구성한다. 미니 Docker 실험에서는 이 구성을 직접 만든다.
...
| 요소 | 작업 |
| --- | --- |
| network namespace | `ip netns add` 또는 process PID namespace 기준으로 생성 |
| veth pair | host 쪽과 container 쪽 가상 NIC 생성 |
| bridge | host에 L2 bridge 생성 |
| IP/route | container IP, gateway, default route 설정 |
| NAT | 외부 통신이 필요하면 MASQUERADE 규칙 추가 |
...
| 질문 | 판단 |
| --- | --- |
| namespace만 이해하려는가 | `unshare --uts --pid --mount --mount-proc`부터 시작 |
| rootfs 전환을 이해하려는가 | `chroot`로 시작하고 `pivot_root`는 별도 실험 |
| resource limit을 이해하려는가 | cgroup v2 실험용 subtree를 만들고 `pids.max`, `memory.max`부터 확인 |
| network까지 직접 만들 것인가 | root 권한, veth/bridge/NAT 정리 계획이 필요 |
| 보안 runtime까지 구현하려는가 | seccomp, capability, LSM까지 들어가므로 학습 범위를 분리 |
